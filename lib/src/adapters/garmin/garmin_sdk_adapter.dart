...
class GarminAdapter implements WearAdapter {
  ...
  Future<void> ensurePermissions() async {
    ...
    // Garmin SDK doesn't require system permissions like HealthKit
    // It uses Bluetooth which is handled by the SDK
    // We just need to ensure the SDK is initialized

    final available = await GarminPlatformChannel.isAvailable();
    if (!available) {
      throw DeviceUnavailableError(
        'Garmin SDK is not available on this device',
      );
    }

    // Check if we have any paired devices
    final pairedDevices = await _deviceManager.getPairedDevices();
    if (pairedDevices.isEmpty) {
      throw DeviceUnavailableError(
        'No Garmin devices paired. Please pair a device first.',
      );
    }
  }
